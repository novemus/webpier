...
BOOST_AUTO_TEST_CASE(client)
{
    ...
    webpier::service foo { "foo", peer, "127.0.0.1:1234", webpier::default_dht_bootstrap, true, false };
    webpier::service bar { "bar", peer, "127.0.0.1:5678", webpier::default_dht_bootstrap, true, false };

    BOOST_REQUIRE_NO_THROW(context->add_export_service(foo));

    boost::process::child server(boost::filesystem::path(SLIPWAY_EXEC), home.string());
